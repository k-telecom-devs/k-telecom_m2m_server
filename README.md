...
## **Обновить пароль(Метод нуждается в лучшем тестировании т.к. может считаться не безопасным)**
...
отсылает шестизначный код доступа на почту пользователя. Приложение получит md5 hash этого кода
```
POST /api/mail
...
При этом запросе пользователь в дальнейшем будет считаться верифицированным()
```
GET /api/confirm
```
**Возвращает**
