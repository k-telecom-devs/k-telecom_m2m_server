...
## **Сгенерировать код подтверждения изменения пароля.**
Сервер отправит на почту код подтверждения. 
```
POST /api/password-reset
    email:(Почта на которую придет письмо),
    name:(Имя пользователя. Используется для дополнительной верификации)
```
...
## **Изменить пароль.**
Сервер сервер изменит пароль, если будет введен правильный код подтверждения.
```
POST /api/password-reset
    email:(Пользователя, на которую придет письмо),
...
Изменение пароля пользователя(Пользователь будет считаться не верифицированным)
```
