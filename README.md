...
## **"Забыл пароль"**
Метод отправляет письмо на указанную почту уже созданного пользователя со случайным шестизначным кодом подтверждения и ссылкой, где надо ввести код. Если код верен, то пароль успешно изменен.
```
...
```
{
    "message": "Mail send, hash generate"
}
```
<br>

## **Регистрация пользователя.**
Внесение пользователя в базу данных. При успешном выполнении запроса, на почту пользователя отправляется письмо с подтверждением
```
POST /api/register
    name:(имя нового пользователя),
    email:(Почта нового пользователя),
    password:(Пароль нового пользователя)
    phone_number:(Номер телефона нового пользователя)
```
**Возвращает**
...
При этом запросе пользователь в дальнейшем будет считаться верифицированным. Ссылка с запросом генерируется автоматически и отправляется на почту при регистрации пользователя.
```
GET /api/confirm
    fbcc689837324a00d4aa9365a7458715:(Принимает в себя user hash, который генерируется при регистрации и изменении настроек пользователя)
```
**Возвращает**
```
{
...
# Запросы для верифицированных пользователей
...
```
{
    "message": [
        {
            "id": 1,
            "created_at": "2022-10-25T04:18:33.000000Z",
            "updated_at": "2022-10-25T04:18:33.000000Z",
            "device_type": "hub"
        },
        {
            "id": 2,
            "created_at": "2022-10-25T04:18:39.000000Z",
            "updated_at": "2022-10-25T04:18:39.000000Z",
            "device_type": "temp"
        }
    ]
}
...
Сервер отправит все дневные средние значения 
```
GET /api/daily-stat
    mac:(мак адресс устройства, чью статистику вы хотите получить)
```
**Возвращает**
```
[
...
Сервер отправит все месячные средние значения
```
GET /api/monthly-stat
    mac:(мак адресс устройства, чью статистику вы хотите получить)
```
**Возвращает**
```
...
## **Отправить метрику**
Отправить на сервер метрику
```
GET /api/group-metrics
```
**Возвращает**
```
{
    "message": [
        {
            "id": 1,
            "created_at": "2022-11-29T11:48:22.000000Z",
            "updated_at": "2022-11-29T11:48:22.000000Z",
            "user_id": 7,
            "group_id": 1
        }
    ]
...
## **Получить метрику**
Получить метрику пользователя
```
GET /api/group-metrics
    group_id:(группа, по которой перезодит пользователь)
```
**Возвращает**
```
{
